# Tree test script tree_test.pl ver 1.0
# An implementation of modified ARC analysis method (sensu Fitzpatrick & Turelli 2006)
...
my $output_file = "20150221_prueba.csv";
# log file (will be rewritten each time)
my $logfile = "20150221_prueba_log.log";
# ArcGIS python path
...
open ( LOG, ">$logfile" ) || die "\nCan't open log file! $0.\n";
open ( STDERR, ">>$logfile" );
open( DATAFILE, ">$output_file" ) || die "\nCan't open output file $output_file : $0.\n";
...
while( my $binary_tree = $treeio->next_tree ) {
    ...
    # the analysis works with binary rooted trees only, so forcing binary tree with arbitrary balanced branch splitting
    $binary_tree->force_binary();

    my $binary_size = $binary_tree->number_nodes;
    my $root = $binary_tree->get_root_node;

    print "Processing tree no. " . $tree_number . " with total number of nodes: " . $original_size . "\n";
    ...
    # if the nexus file has already been started, we don´t need to output another header and translate table
    if( $tree_number > 1 ) {
        $treeout->translate_node(undef);
        $treeout->header(undef);
    }

    # save copy of new tree (after converting to binary and setting node names)
    $treeout->write_tree($binary_tree);

    print "Modified tree no. " . $tree_number . ":\n" . $binary_tree->as_text; 
